...
class Bilibili(ContentPlatform):
    ...
    def fetch_user(self, username: str) -> dict | None:
        ...
        try:
            ...
            self.logger.error(
                f"Error fetching Bilibili channel info for @{username}: {e}"
            )
            return None
    ...
    def parse_channel_title_from(tree):
        element = tree.xpath(
            '/html/body/div[1]/div[1]/div/section/main/div/div/div/div[1]/div[2]/h1/text()'
        )
        if len(element) > 0:
            return element[0]
        return None
    ...
    def parse_profile_image_url_from(tree):
        element = tree.xpath(
            '/html/body/div[1]/div[1]/div/section/main/div/div/div/div[1]/div[1]/picture/source'
        )
    ...
    def parse_followers_count_from(tree):
        element = tree.xpath(
            '/html/body/div[1]/div[1]/div/section/main/div/div/div/div[1]/div[2]/div/p[2]/a'
        )
        if len(element) > 0:
    ...
    def parse_following_count_from(tree):
        element = tree.xpath(
            '/html/body/div[1]/div[1]/div/section/main/div/div/div/div[1]/div[2]/div/p[1]/a'
        )
        if len(element) > 0:
            return int(value_to_float(element[0].text))
        return 0
    ...
    def parse_likes_count_from(tree):
        element = tree.xpath(
            '/html/body/div[1]/div[1]/div/section/main/div/div/div/div[1]/div[2]/div/p[3]/span[1]'
        )
        if len(element) > 0:
            return int(value_to_float(element[0].text))
        return 0
